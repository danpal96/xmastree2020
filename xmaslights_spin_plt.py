...
class PlotPixels(UserList):
    def __init__(self, coords, update_interval=0.1):
        super().__init__()
        self.coords = coords
    ...
    def colors(self):
        hex_colors = []
        for g, r, b in self:
            hex_colors.append(rgb2hex([r / 255, g / 255, b / 255]))
        return hex_colors

    def show(self):
        self.ax.clear()
        self.ax.scatter(self.x, self.y, self.z, color=self.colors)
...
def xmaslight():
    ...
    pixels = PlotPixels(coords, update_interval=0.1)
